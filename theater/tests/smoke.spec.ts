...
test("every control changes observable state", async ({ page }) => {
	...
	const rendered = await page
		...
		.evaluateAll((elements) => elements.map((element) => element.getAttribute("data-control")));
	...
	await expect
		.poll(async () => page.getByTestId("virtual-time").textContent(), { timeout: 15_000 })
		.not.toBe("0s / 60s");
...
test("cluster size, node selection, stepping, and fault readouts are live", async ({ page }) => {
	...
	await canvas.click({
		position: {
			x: box.width * 0.48 + Math.cos(angle) * radius,
			y: box.height * 0.51 + Math.sin(angle) * radius,
		},
	});
...
test("reduced motion disables autoplay", async ({ page }) => {
	...
	await expect
		.poll(() => page.evaluate(() => localStorage.getItem("crash-course-motion")))
		.toBe("off");
	await page.reload();
	await expect(page.locator('[data-control="motion-preference"]')).toHaveAccessibleName(
...
test("text mirror reports role changes", async ({ page }) => {
	...
	await expect(page.locator(`#topology-mirror tbody tr:nth-child(${leader})`)).toContainText(
		"leader",
	);
});
...
test("wasm state and trace pages obey the ABI byte cap", async ({ page }) => {
	...
	const result = await page.evaluate(async () => {
		...
		try {
			...
			return {
				stateBytes: new TextEncoder().encode(state).length,
				pageBytes: new TextEncoder().encode(page).length,
			};
		} finally {
			handle.free();
		}
...
test("museum ABI compatibility is explicit and bounded", async ({ page }) => {
	...
	await page.addInitScript(() => {
		...
		window.fetch = (input, init) =>
			String(input).includes("exhibits/manifest.json")
				? Promise.resolve(
						new Response(JSON.stringify({ schema_version: 99, build: "future", exhibits: [] }), {
							status: 200,
