...
type WasmNode = { id: number; status: string; role: Role; term: number; commit: number; applied: number; durable_bytes: number; disk_service_delay_ms: number; clock_offset_ms: number; log_tail: number[] };
type WasmState = { theater_abi: number; virtual_time_ns: number; history_len: number; completed_operations: number; had_leader: boolean; link_drop_percent: number; checkpoint_count: number; checkpoint_bytes: number; nodes: WasmNode[] };
...
function readTrace(module: WasmModule, handle: SimHandle): TraceFixture {
  ...
  let cursor = 0n;
  for (;;) {
    const page = JSON.parse(module.tracePage(handle, cursor, 512)) as { events: TraceEvent[]; next_cursor: number; done: boolean };
...
function advanceWithCheckpoints(runtime: WasmRuntime, targetNs: number): WasmState {
  ...
  while (state.virtual_time_ns < targetNs) {
    ...
    state = JSON.parse(runtime.module.step(runtime.handle, BigInt(nextBoundary - state.virtual_time_ns))) as WasmState;
    if (state.virtual_time_ns % CHECKPOINT_INTERVAL_NS === 0 && !runtime.checkpoints.has(state.virtual_time_ns)) {
...
const LESSONS: Record<Exclude<LessonName, "free">, { title: string; chapter: string; action: FaultSpec }> = {
...
export default function App() {
  ...
  const memoryCheckpoints = useMemo(() => checkpointTimes.map((time) => time / 60_000_000_000), [checkpointTimes]);
  ...
  const clockSkewMs = wasmState?.nodes.find((node) => node.id === selectedNode.id)?.clock_offset_ms ?? 0;
  ...
  useEffect(() => {
    ...
    loadMuseum()
      ...
      .catch((error: unknown) => {
        if (!cancelled) {
          ...
          setMuseumError(`Museum import failed. ${error instanceof Error ? error.message : String(error)}`);
        }
      });
    ...
    void (async () => {
      try {
        ...
        if (state.theater_abi !== 2) throw new Error(`unsupported Theater ABI ${state.theater_abi}`);
        ...
        if (!cancelled) {
          ...
          setSimError(`The simulator failed to initialize; controls are paused. ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    })();
    ...
    const timer = window.setInterval(() => {
      ...
      if (runtime) {
        try {
          ...
          const state = advanceWithCheckpoints(runtime, Math.min(60_000_000_000, current.virtual_time_ns + stepNs));
          setWasmState(state);
  ...
  }, [reducedMotion, wasmState, leaderNode]);
  ...
  function inject(action: FaultSpec) {
    ...
    try {
      ...
      setFaultAnnouncement(`Fault completed: ${action.action}${action.node ? ` on node ${action.node}` : ""}.`);
      setSimError("");
    } catch (error) {
      setPlaying(false);
      setSimError(error instanceof Error ? error.message : String(error));
    ...
    queueMicrotask(() => {
      ...
      try {
        ...
        if (nearest === undefined || targetNs - nearest > CHECKPOINT_INTERVAL_NS) {
          throw new Error("No complete checkpoint is available within the five-second replay horizon.");
        }
        runtime.module.restore(runtime.handle, runtime.checkpoints.get(nearest)!);
  ...
  async function proveDeterminism() {
    ...
    for (let pass = 0; pass < 2; pass += 1) {
      ...
      try {
        faults.forEach((action) => runtime.module.inject(handle, JSON.stringify(action)));
    ...
    setTraceHash([...new Uint8Array(digest)].map((byte) => byte.toString(16).padStart(2, "0")).join("").slice(0, 12));
  ...
  return (
    <main className={`shell${embedded ? " embed" : ""}${reducedMotion ? " reduce-motion" : ""}`}>
      <header className="topbar">
        ...
        <button className="quiet-button" data-control="play" disabled={reducedMotion || !wasmState} title={reducedMotion ? "Autoplay is disabled by the motion preference." : !wasmState ? "The simulator is not ready." : undefined} onClick={() => setPlaying((value) => !value)}>{playing ? "PAUSE" : "PLAY"}</button>
        <button className="quiet-button" data-control="motion-preference" aria-label={`Motion preference: ${motionPreference}`} onClick={() => setMotionPreference(nextMotionPreference(motionPreference))}>MOTION {motionPreference.toUpperCase()}</button>
      </header>
      {simError && <p className="sim-error" role="alert">SIMULATOR PAUSED · {simError}</p>}
      <span className="visually-hidden" data-testid="live-wasm-handles">{liveHandles}</span>
      <section className="control-strip">
        ...
        <button className="outline-button" data-control="heal-all" disabled={!partitioned || !wasmState} title={!partitioned ? "No partition is active." : undefined} onClick={() => { setPartitioned(false); inject({ action: "heal" }); }}>HEAL ALL</button>
        ...
        <button className="accent-button" data-control="kill-leader" disabled={!leaderNode || !wasmState} title={!leaderNode ? "No elected leader can be crashed." : undefined} onClick={killLeader}>KILL LEADER <span>k</span></button>
      </section>
      <section className="workspace">
        <aside className="chaos-panel panel">
          <div className="panel-kicker">CHAOS PALETTE</div>
          <button data-control="crash-selected" disabled={!wasmState} title={!wasmState ? "The simulator is not ready." : undefined} onClick={() => inject({ action: "crash", node: selectedNode.id })}><span className="icon danger">✕</span><span><b>Crash node</b><small>choose a voter</small></span></button>
          ...
          <button data-control="heal-palette" disabled={!partitioned || !wasmState} title={!partitioned ? "No partition is active." : undefined} onClick={() => { setPartitioned(false); inject({ action: "heal" }); }}><span className="icon calm">⌁</span><span><b>Heal all</b><small>restore every link</small></span></button>
          ...
          <div className="slider-label"><span>CLOCK SKEW · n{selectedNode.id}</span><b data-testid="clock-skew-value">{clockSkewMs} ms</b></div><input data-control="clock-skew" type="range" min="0" max="100" aria-label="Clock skew" value={clockSkewMs} onChange={(event) => inject({ action: "clock-skew", node: selectedNode.id, offset_ms: Number(event.target.value) })} />
        ...
        <section className="timeline panel"><div className="timeline-head"><span className="panel-kicker">TIMELINE / RE-EXECUTION</span><span className="timeline-actions"><button data-control="previous-event" aria-label="Previous event" title="Previous event" onClick={() => stepToMarker(-1)}>◀</button><button data-control="timeline-play" disabled={reducedMotion || !wasmState} title={reducedMotion ? "Autoplay is disabled by the motion preference." : undefined} aria-label={playing ? "Pause timeline" : "Play timeline"} onClick={() => setPlaying((value) => !value)}>{playing ? "Ⅱ" : "▶"}</button><button data-control="next-event" aria-label="Next event" title="Next event" onClick={() => stepToMarker(1)}>▶</button><span data-testid="virtual-time">{Math.round(virtualTime * 60)}s / 60s</span></span></div><div className="timeline-track"><input data-control="timeline" className="timeline-range" type="range" min="0" max={maxScrubSeconds} step="1" aria-label="Timeline" aria-valuemax={maxScrubSeconds} aria-valuetext={`${Math.round((playing ? virtualTime : checkpoint) * 60)} virtual seconds`} value={Math.min(maxScrubSeconds, Math.round((playing ? virtualTime : checkpoint) * 60))} onChange={(event) => scrubTo(Number(event.target.value) / 60)} />{markers.map((marker) => <button data-control="timeline-marker" key={marker.seq} className={`marker ${marker.kind}`} style={{ left: `${marker.t * 100}%` }} aria-label={marker.note} title={marker.note} onClick={() => scrubTo(marker.t)} />)}<div className="playhead" style={{ left: `${(playing ? virtualTime : checkpoint) * 100}%` }} /></div><span className="visually-hidden" data-testid="restore-from-ns">{lastRestoreFrom}</span><span className="visually-hidden" data-testid="replay-ns">{lastReplayNs}</span><div className="timeline-labels"><span>00:00</span><span>captured events</span><span>60:00</span></div><div className="checkpoint-row">MEMORY CHECKPOINTS {memoryCheckpoints.length === 0 ? <span className="subtle">none captured</span> : memoryCheckpoints.map((value) => <button data-control="checkpoint" key={value} onClick={() => scrubTo(value)} className={checkpoint === value ? "selected-checkpoint" : ""}>{Math.round(value * 60)}s</button>)}</div></section>
      <section className="museum panel"><div className="timeline-head"><span><span className="panel-kicker">MUSEUM / VERIFIED EXHIBITS</span><small>manifest build {museum.build} · ABI {museum.theater_abi}</small></span><span className="museum-tools"><select data-control="museum-category" aria-label="Museum category" value={museumFilter} onChange={(event) => setMuseumFilter(event.target.value)}><option value="all">all</option><option value="raft">raft</option><option value="wal">wal</option><option value="store">store</option><option value="membership">membership</option><option value="checker">checker</option></select><span className="status-chip">{visibleExhibits.length} LOADED</span></span></div>{museumError ? <p className="museum-empty" role="alert">{museumError}</p> : museum.exhibits.length === 0 ? <p className="museum-empty">No verified failure exhibits are published yet. The wing stays empty until a real shrunk trace earns a pinned build.</p> : visibleExhibits.length === 0 ? <p className="museum-empty">No exhibits match this category.</p> : <div className="exhibit-grid">{visibleExhibits.map((exhibit) => <button data-control="museum-exhibit" key={exhibit.id} className="exhibit-card" onClick={() => setSeed(exhibit.seed)}><b>{exhibit.title}</b><small>{exhibit.kind} · {exhibit.verdict} · {exhibit.anomaly} · ABI {exhibit.theater_abi}{exhibit.readonly ? " read-only" : ""} · {exhibit.chapters.length} chapters</small></button>)}</div>}</section>
      <footer><span>THEATER ABI 2 · TRACE v1 · BUILD fixture</span><span>Keyboard: <kbd>Tab</kbd> controls · <kbd>Space</kbd> play/pause · <kbd>K</kbd> kill leader · motion {motionPreference} · <button data-control="share" className="link-button" onClick={shareScenario}>{shared ? "URL copied ✓" : "Share this scenario ↗"}</button></span></footer>
    </main>
  );
}
