...
export function state(handle) {
    ...
    try {
        ...
        const ret = wasm.state(handle.__wbg_ptr);
        deferred1_0 = ret[0];
        deferred1_1 = ret[1];
 ...
 * @returns {SimHandle}
 */
export function init(spec_json) {
 ...
 */
export function inject(handle, action_json) {
    _assertClass(handle, SimHandle);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('cc_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
