...
export function parseMuseum(value: unknown): MuseumManifest {
  ...
  return {
    ...
    build: typeof manifest.build === "string" && manifest.build.length > 0 ? manifest.build : "unknown",
    exhibits: manifest.exhibits.map((candidate, index) => adaptExhibit(candidate, manifestAbi, index)),
  };
}
...
function adaptExhibit(value: unknown, manifestAbi: 1 | 2, index: number): Exhibit {
  ...
  if (!isPositiveSafeInteger(horizon) || !isPositiveSafeInteger(interval)) {
    throw new Error(`Museum exhibit ${value.id} must declare a finite horizon and checkpoint interval.`);
  }
...
function isRawExhibit(value: unknown): value is Omit<Exhibit, "anomaly" | "theater_abi" | "horizon_ns" | "checkpoint_interval_ns" | "readonly"> & { anomaly?: string } {
