...
entry from the current term, and sends the leader no-op before serving a read
barrier. Each rule has a named test because the failure mode is more memorable
than a generic “Raft works” test.
...
use learners and a joint configuration so both majorities are explicit.
