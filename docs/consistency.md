...
majority and the state machine applies it. Reads use the leader's ReadIndex
barrier and wait for `applied_index` to reach that index; followers return an
explicit not-leader result rather than pretending to provide a local read.
