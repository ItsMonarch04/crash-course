...
cargo test --locked -p cc-swarm trap_default_build_enables_no_kata
cargo test --locked -p cc-swarm trap_kata_features_are_mutually_exclusive

pair_log="$repo_root/target/kata-mutual-exclusion.log"
mkdir -p "$(dirname "$pair_log")"
if cargo check --locked -p cc-swarm --features kata01,kata02 >"$pair_log" 2>&1; then
...
for pair in cc-raft:kata01 cc-raft:kata02 cc-host:kata03 cc-store:kata04 cc-cluster:kata05; do
  cargo clippy --locked -p "${pair%%:*}" --all-targets --features "${pair##*:}" -- -D warnings
done
...
cargo test --locked -p cc-store --features kata04 trap_kata_04_tombstone_gc_is_found_within_budget
...
for kata in kata01 kata02 kata03 kata04 kata05; do
  cargo test --locked -p cc-swarm --features "$kata" trap_active_kata_is_visible_in_trace_and_artifact_type
done
